...
output "cgw_id" {
  ...
  value = tomap({
    for k, v in aws_customer_gateway.this : k => v.id
  })
}

#vpn with cgw
output "cgw_vpn_connections" {
  description = "Map of VPN Connections"
  value       = aws_vpn_connection.cgw
}

#VGW Id
output "vgw_id" {
  value = tomap({
    for k, v in aws_vpn_gateway.this : k => v.id
  })
  description = "VGW ID"
}

#vpn with vgw
output "vgw_vpn_connections" {
  description = "Map of VPN Connections"
  value       = aws_vpn_connection.vgw
}